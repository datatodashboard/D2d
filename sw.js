...
self.addEventListener('activate',event=>{
  event.waitUntil(
    caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('crack-sql-')&&k!==CACHE).map(k=>caches.delete(k))))
  ...
  event.respondWith((async()=>{
    ...
    if(isNavigation){
      try {
        ...
        if(netResp&&netResp.ok)cache.put(new URL('index.html',ROOT).href,netResp.clone());
        return netResp;
      }catch(e){
        const cached=await cache.match(new URL('index.html',ROOT).href);
